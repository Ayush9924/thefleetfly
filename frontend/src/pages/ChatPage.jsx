...
export default function ChatPage() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const response = await api.get("/users/contacts/available");
        const contactsByRole = response.data;
        
        // Flatten and combine all users
        const allUsersList = [
          ...(contactsByRole.driver || []),
          ...(contactsByRole.manager || []),
          ...(contactsByRole.admin || []),
        ].filter((u) => u._id !== user?._id);
        
        setAllUsers(allUsersList);
      } catch (error) {
        console.error("Error fetching users:", error);
        // Fallback: try to fetch from drivers endpoint
        try {
          const driversRes = await api.get("/drivers");
          setAllUsers(driversRes.data?.filter((u) => u._id !== user?._id) || []);
        } catch (fallbackError) {
  ...
  const getOtherUserName = (conv) => {
    if (!conv || !conv.participants || conv.participants.length === 0) return "Unknown";
    const other = conv.participants.find((p) => 
      (p.userId?._id !== user?._id) || (p.userId && p.userId !== user?._id)
    );
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  {!isConnected && (
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 mt-3 bg-yellow-100 text-yellow-800 rounded-full text-sm font-bold">
                      <Zap className="h-4 w-4" />
                      Connecting...
                    </div>
                  )}
                  {totalMessages > 0 && isConnected && (
                    <motion.div
              ...
              <div className="flex flex-wrap gap-3">
                ...
                >
                  <button
                    className="border-2 border-blue-300 text-blue-600 hover:bg-blue-50 font-semibold px-4 py-3 rounded-lg lg:hidden"
                      ...
                      <div className="mt-4 max-h-48 overflow-y-auto space-y-2">
                        {loadingUsers ? (
                          ...
                          allUsers.map((u) => {
                            ...
                            const roleColor = roleColors[u.role] || 'bg-gray-100 text-gray-700';
                            ...
                            return (
                              <button
                                ...
                                disabled={loading}
                                className="w-full text-left px-3 py-3 rounded-lg hover:bg-blue-50 transition disabled:opacity-50 bg-white border border-gray-100 hover:border-blue-200"
                              >
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className={`px-2 py-1 rounded text-xs font-semibold ${roleColor}`}>
                                    {u.role}
                                  </span>
                                </div>
                              </button>
              ...
              {selectedConv ? (
                ...
                >
                  ...
                  <p className="text-gray-600 font-medium text-lg max-w-md mb-4">
                    Select a conversation to start chatting in real-time with your team.
                  </p>
